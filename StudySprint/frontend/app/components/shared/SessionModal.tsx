...
export function SessionModal({ goalId, initialMinutes, session, onClose, onSaved }: Props) {
  ...
  return (
    ...
    >
      <div
        className="bg-[#0a0a0a] border border-white/10 rounded-2xl w-full max-w-lg overflow-hidden flex flex-col shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="px-8 py-6 border-b border-white/10 flex items-center justify-between">
          <h3 className="text-sm font-bold tracking-widest uppercase text-zinc-50">
        ...
        <form onSubmit={onSubmit} className="p-8 space-y-8">
          <div className="space-y-4">
            ...
            <div className="relative">
              <input
                ...
                className="w-full bg-transparent border-b border-white/20 pl-8 pr-0 py-3 text-zinc-50 text-xl font-medium focus:outline-none focus:border-[#ccff00] transition-colors rounded-none"
              />
          ...
          <div className="space-y-4">
            ...
            <textarea
              ...
              rows={4}
              className="w-full bg-transparent border border-white/20 p-4 text-zinc-50 text-sm font-light focus:outline-none focus:border-[#ccff00] transition-colors rounded-xl resize-none"
            />
          </div>
          ...
          <div className="flex gap-4 pt-4">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 py-4 px-4 rounded-full text-xs font-bold tracking-widest uppercase text-zinc-50 border border-white/20 hover:border-white/50 transition-colors"
            >
              Cancel
            </button>
            <button
